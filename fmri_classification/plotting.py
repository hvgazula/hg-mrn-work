...
results_folder_list = list(
    filter(os.path.isdir, glob.glob('Results*')))

for folder in results_folder_list:
    code = folder.split('_')[1]
    data_set, features_selected, runs = code.split('-')

    for key in classifier_dict.keys():
        try:
            file_list = sorted([
                os.path.splitext(x)[0]
                for x in glob.glob(os.path.join(folder, key + '/*.dat'))
            ])
            ...
            for index, df in enumerate(df_list):
                ...
                for i in range(1, len(curr_series)):
                    curr_elem = list(
                        set(curr_series[i + 1]) - set(curr_series[i]))
            ...
            for col in index_df.columns:
                ...
                plt.title('{} (Dataset = {}, Features selected = {})'.format(
